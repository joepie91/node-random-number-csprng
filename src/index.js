...
module.exports = function secureRandomNumber(minimum, maximum, cb) {
	return Promise.try(() => {
		...
		if (minimum == null) {
			throw new RandomGenerationError("The 'minimum' argument is null or undefined.");
		}

		if (maximum == null) {
			throw new RandomGenerationError("The 'maximum' argument is null or undefined.");
		}

		if (minimum % 1 !== 0) {
			throw new RandomGenerationError("The 'minimum' argument must be an integer.");
		}

		if (maximum % 1 !== 0) {
			throw new RandomGenerationError("The 'maximum' argument must be an integer.");
		}

		if (!(maximum > minimum)) {
			throw new RandomGenerationError("The 'maximum' argument must be higher than the 'minimum' argument.");
		}
		...
		if (minimum < -9007199254740991 || minimum > 9007199254740991) {
			throw new RandomGenerationError("The 'minimum' argument must be between MIN_SAFE_INTEGER and MAX_SAFE_INTEGER, exclusive.");
		}

		if (maximum < -9007199254740991 || maximum > 9007199254740991) {
			throw new RandomGenerationError("The 'maximum' argument must be between MIN_SAFE_INTEGER and MAX_SAFE_INTEGER, exclusive.");
		}

		let range = maximum - minimum;

		if (range < -9007199254740991 || range > 9007199254740991) {
			throw new RandomGenerationError("The range between the 'minimum' and 'maximum' arguments must be between MIN_SAFE_INTEGER and MAX_SAFE_INTEGER, exclusive.");
		}
